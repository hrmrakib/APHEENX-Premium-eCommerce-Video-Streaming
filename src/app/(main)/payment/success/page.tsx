...
function PaymentSuccessPageComponent() {
  ...
  const [updateProductOrderCaptureMutation] =
    useUpdateProductOrderCaptureMutation();
  const [updateVideoOrderCaptureMutation] =
    useUpdateVideoOrderCaptureMutation();
  ...
  // ✅ Call the correct API based on order_type when the page loads
  useEffect(() => {
    if (!orderId || !orderType) return;

    if (orderType === "product") {
      updateProductOrderCaptureMutation(orderId);
    } else if (orderType === "video") {
      updateVideoOrderCaptureMutation(orderId);
    }
  }, [orderId, orderType]);
